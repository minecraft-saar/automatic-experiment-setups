...
SETUP_DIR=${1:-2021-time-optimization}
mkdir -p $SETUP_DIR
cd $SETUP_DIR
...
if [[ ! -f .setup_complete ]]; then
    ...
    cp ../configs/broker-config-2021-time-optimization.yaml infrastructure/broker/broker-config.yaml

    rm simple-architect/configs/*yaml
    #cp -a ../configs/2021-rerun-one-ins/basic-configs/. simple-architect/configs

    for i in $(ls ../configs/2021-time-optimization/plans/ | grep 'lisp$'); do
	cfg=simple-architect/configs/${i%lisp}yaml
	weights=$(cd ../configs/2021-time-optimization/weights; pwd)/${i%lisp}json
	plan=$(cd ../configs/2021-time-optimization/plans/; pwd)/$i
	cp ../configs/2021-time-optimization/architect.yaml $cfg
...
CREATE DATABASE IF NOT EXISTS TIMEOPTIMIZATION
EOF
