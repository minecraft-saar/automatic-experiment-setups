...
SETUP_DIR=${1:-2021-one-instruction}
mkdir -p $SETUP_DIR
cd $SETUP_DIR
...
if [[ ! -f .setup_complete ]]; then
    ...
    cp -a ../configs/2021-one-instruction/configs/. simple-architect/configs

    for i in $(ls ../configs/2021-one-instruction/ | grep 'plan$'); do
	cfg=simple-architect/configs/${i%plan}yaml
	weights=$(cd ../configs/2021-one-instruction/weights; pwd)/${i%plan}json
	plan=$(cd ../configs/2021-one-instruction/; pwd)/$i
	cp ../configs/2021-one-instruction/architect.yaml $cfg
