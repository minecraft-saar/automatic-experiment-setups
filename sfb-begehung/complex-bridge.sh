...
if [[ ! -f .setup_complete ]]; then
    ...
    cp ../../configs/broker-config-test-bridge-fancy.yaml infrastructure/broker/broker-config.yaml

    rm simple-architect/configs/*yaml
    #cp -a ../configs/2021-rerun-one-ins/basic-configs/. simple-architect/configs

    for i in $(ls ../../configs/uniform-time/plans/ | grep 'complex-bridge.lisp$'); do
